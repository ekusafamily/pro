...
const STORE_CONFIG = {
  ...
  pin: "P051123456X",
  till: "4209800",
  vatRate: 0.00, // Assuming 0 for now as per previous context or lack thereof
};
...
export const Sales: React.FC = () => {
  ...
  const vatAmount = 0; // Keeping simple for now
    ...
    try {
      ...
      for (const item of cart) {
        ...
        if (saleType === SaleType.CASH) {
          // Simplified: Assume full payment if Cash is selected (or sufficient funds)
          // If partial cash payment exists in system, we'd need logic.
          // Here we assume Cash means "Paid in Full" for the items.
          itemPaid = itemTotal;
        }
        ...
        await store.recordSale({
          ...
          total_price: itemTotal, // store.recordSale ignores this but good for ref
          date: timestamp,
          loyalty_points: 0,
          change_amount: 0 // Don't record change per item to avoid skewing sums
          ...
          <div className="p-6 bg-slate-50 border-t border-slate-100 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <span className="text-3xl font-black text-slate-800">KES {grandTotal.toLocaleString()}</span>
              </div>
            </div>
